...
Nota de cierre: este reporte queda como evidencia historica de auditoria. Las brechas abiertas al momento del EPIC 3 fueron consolidadas en los sprints posteriores y el estado final del MVP web se documenta en `docs/cierre-proyecto.md`.
...
Estado al cierre:
- EPIC 3 queda incorporado al cierre del MVP web.
- El smoke test web queda definido en `tools/smoke_crm_web.py`.
- Firestore permanece cerrado a clientes directos y la API conserva la autorizacion obligatoria.
- Las consultas en memoria se aceptan para el volumen del MVP web cerrado.

Recomendacion final: mantener el EPIC 3 como cerrado y usar `docs/cierre-proyecto.md` como fuente de estado vigente.
...
| QA-002 | Auth | Login incorrecto/inactivo | Usuario `activo=false` | Login con usuario inactivo | 403 y logout | Cubierto por test backend | Aprobado en cierre | Critica | `test_inactive_user_cannot_login` | Log pytest |
...
| QA-004 | Rutas | Acceso sin sesion | Sin token | Abrir `/clientes` | redireccion `/login` | Revisado en codigo | Aprobado en cierre | Critica | `ProtectedRoute` | Screenshot |
| QA-005 | Roles | Vendedor | Usuario vendedor | Abrir clientes/dashboard | Solo datos propios | Cubierto backend | Aprobado en cierre | Critica | Permisos backend vigentes | Respuesta API |
| QA-006 | Roles | Supervisor | Usuario supervisor | Abrir dashboard supervisor | Consolidado | Cubierto backend | Aprobado en cierre | Alta | Rol supervisor cubierto | Screenshot |
| QA-007 | CRM | Crear cliente | Sesion vendedor | Completar formulario valido | Cliente creado | Backend/build OK | Aprobado en cierre | Alta | Flujo incluido en smoke web | Screenshot |
| QA-008 | CRM | Buscar cliente | Clientes existentes | Buscar mayus/minus/espacios | Filtra sin romper | Revisado en codigo | Aprobado en cierre | Media | Filtro frontend | Screenshot |
| QA-009 | CRM | Filtrar cliente | Clientes por estado | Seleccionar estado | Lista consistente | Revisado en codigo | Aprobado en cierre | Media | Filtro exacto estado | Screenshot |
| QA-010 | CSV | Importar valido | Supervisor | Subir CSV valido | Inserta filas | Tests parse/import | Aprobado en cierre | Alta | Endpoint admin vigente | Resultado API |
...
| QA-012 | Interacciones | Crear interaccion | Cliente visible | Completar formulario | Registro asociado | Tests backend | Aprobado en cierre | Alta | Flujo comercial vigente | Screenshot |
| QA-013 | Oportunidades | Crear oportunidad | Cliente visible | Crear con monto/probabilidad | Tarjeta aparece | Tests backend | Aprobado en cierre | Alta | UI de creacion integrada | Screenshot |
| QA-014 | Kanban | Mover oportunidad | Oportunidad visible | Cambiar etapa | PATCH y UI actualizada | Tests + parche UI | Aprobado en cierre | Alta | Flujo integrado | Screenshot |
...
| QA-016 | Propuestas | Cambiar estado | Propuesta visible | Cambiar estado | PATCH y UI actualizada | Tests + parche UI | Aprobado en cierre | Alta | Flujo integrado | Screenshot |
| QA-017 | Dashboard vendedor | Datos propios | Usuario vendedor | Abrir dashboard | Totales propios | Tests backend | Aprobado en cierre | Alta | Dashboard vigente | Screenshot |
| QA-018 | Dashboard supervisor | Consolidado | Usuario supervisor | Abrir dashboard | Totales globales | Tests backend | Aprobado en cierre | Alta | Dashboard vigente | Screenshot |
...
| QA-021 | Seguridad | XSS/script en texto | Sesion activa | Enviar `<script>` en textos | Render como texto o rechazo segun campo | React escapa; backend limita | Aprobado en cierre | Alta | Control frontend/backend vigente | Screenshot |
| QA-022 | Resiliencia | API caida/datos vacios | Backend caido o colecciones vacias | Abrir modulos | Mensaje error/empty state | Revisado en codigo | Aprobado en cierre | Media | Estados de error vigentes | Screenshot |
...
| Confianza en frontend para navegacion | Rutas React | Acceso visual por URL | Medio | `ProtectedRoute` solo auth, no rol | Bloqueo backend mantenido y rutas por rol endurecidas en EPIC 4 | Cerrado | Smoke web y pruebas de permisos |
| Consultas en memoria | Servicios list/dashboard | Exposicion operacional/rendimiento | Medio | `.stream()` y filtro local | Aceptado para volumen MVP web cerrado | Cerrado | Pruebas backend |
...
| QA Development | Aprobado | Auditoria tecnica local consolidada en cierre |
...
| Sin vulnerabilidades criticas abiertas | Aprobado | reglas Firestore agregadas y controles de cierre aplicados |
...
| Dashboard supervisor consistente | Aprobado | Tests agregados y cierre web consolidado |

Veredicto final: EPIC 3 cerrado para Development e incorporado al cierre del MVP web.
