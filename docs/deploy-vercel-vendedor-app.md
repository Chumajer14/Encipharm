...
El proyecto `enciapp` usa `/api` y `vendedor-app/api/[...path].js` proxyea `/api/*` hacia `https://encipharm.onrender.com/*`.
...
- Abrir `Enci Chat` desde la barra inferior y enviar una consulta contra `/rag/chat`.
